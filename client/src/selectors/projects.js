...
const findProjectById = (projects, projectID) => projects.find(project => project.id === projectID)
...
const filterProjectsByCurrentUserId = (projects, userProjects, currentUserID) => {
  ...
  for (const userProject of currentUserUserProjects) {
    const foundProject = projects.find(proj => proj.id === userProject.projectID)
    if (foundProject && !currentUserProjects.find(proj => proj.id === foundProject.id)) {
...
const findFirstProjectIDByCurrentUserId = (projects, userProjects, currentUserID) => {
  const currentUserUserProjects = userProjects.filter(userProj => userProj.userID === currentUserID)
  const currentUserProjects = projects.filter(project =>
    currentUserUserProjects.map(userProj => userProj.projectID).includes(project.id)
  )
  return currentUserProjects[0] ? currentUserProjects[0].id : 1
}

const makeSelectProjectById = createSelector([selectProjects, selectId], findProjectById)

const makeSelectDeduplicatedProjects = createSelector([selectProjects], dedupProjects)
